...
async function replaceState(client,state){
  ...
  try{
    await client.query("delete from payments"); await client.query("delete from credits");
    await client.query("delete from maintenance_tasks"); await client.query("delete from contracts");
    await client.query("delete from tenants"); await client.query("delete from properties");
    ...
    for(const x of state.payments||[]){const cid=cMap.get(x.contractId);if(cid)await client.query(`insert into payments(id,contract_id,period,amount,payment_date,method,notes) values($1,$2,$3,$4,$5,$6,$7)`,[uid(x.id),cid,x.period,Number(x.amount||0),x.date,x.method||null,x.notes||null]);}
    for(const x of state.credits||[]){const cid=cMap.get(x.contractId);if(cid)await client.query(`insert into credits(id,contract_id,amount,payment_date,note) values($1,$2,$3,$4,$5)`,[uid(x.id),cid,Number(x.amount||0),x.date,x.note||null]);}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==="PUT"){
      ...
      const error=permissionError(a.user.role,before,state);
